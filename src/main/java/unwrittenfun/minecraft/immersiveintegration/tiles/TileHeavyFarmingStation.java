...
import unwrittenfun.minecraft.immersiveintegration.client.gui.GuiHeavyFarmingStation;
...
public class TileHeavyFarmingStation extends TileEntity implements IMultiblockTile, IGuiProvider, ISidedInventory, IFluidHandler {
  public int[] offset;
  ...
  public void doAutoOutput(ForgeDirection side) {
    ...
    if (tileEntity instanceof IInventory && !(tileEntity instanceof TileHeavyFarmingStation)) {
  ...
  public Object getContainer(int id, EntityPlayer player) {
    return new ContainerHeavyFarmingStation(this, player.inventory);
  }

  @Override
  public Object getGuiContainer(int id, EntityPlayer player) {
    return new GuiHeavyFarmingStation((ContainerHeavyFarmingStation) getContainer(id, player));
  }
  ...
  public TileHeavyFarmingStation getMaster() {
    if (!isFormed() || isMaster() || !hasWorldObj()) {
      return null;
    }
    TileEntity master = worldObj.getTileEntity(xCoord - offset[0], yCoord - offset[1], zCoord - offset[2]);
    return master instanceof TileHeavyFarmingStation ? (TileHeavyFarmingStation) master : null;
  }
  ...
  public ItemStack getStackInSlot(int slot) {
    TileHeavyFarmingStation master = getMaster();
